...
class VLMGRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        def fix_config_dict_issues(config):
            """Fix config dict issues for Qwen2.5-VL models"""
            # Fix text_config if it's a dict
            if hasattr(config, 'text_config') and isinstance(config.text_config, dict):
                try:
                    from transformers import Qwen2Config
                    # Remove problematic keys before creating Qwen2Config
                    text_config_dict = config.text_config.copy()
                    text_config_dict.pop('image_token_id', None)
                    text_config_dict.pop('video_token_id', None) 
                    text_config_dict.pop('vision_start_token_id', None)
                    text_config_dict.pop('vision_end_token_id', None)
                    text_config_dict.pop('vision_token_id', None)
                    config.text_config = Qwen2Config(**text_config_dict)
                    print("Fixed text_config from dict to Qwen2Config")
                except Exception as e:
                    print(f"Warning: Could not fix text_config: {e}")
            
            # Fix vision_config if it's a dict  
            if hasattr(config, 'vision_config') and isinstance(config.vision_config, dict):
                try:
                    from transformers.models.qwen2_vl.configuration_qwen2_vl import Qwen2VLVisionConfig
                    config.vision_config = Qwen2VLVisionConfig(**config.vision_config)
                    print("Fixed vision_config from dict to Qwen2VLVisionConfig")
                except Exception as e:
                    print(f"Warning: Could not fix vision_config: {e}")
            
            return config

        model_cls = self.vlm_module.get_model_class(model_id, model_init_kwargs)
        
        # Initialize variables for config fixing
        model_init_kwargs_fixed = None
        fixed_config = None
        
        # Fix config loading issues for checkpoints
        try:
            model = model_cls.from_pretrained(model_id, **model_init_kwargs)
        except (AttributeError, TypeError) as e:
            error_str = str(e)
            if "'dict' object has no attribute 'to_dict'" in error_str or "text_config" in error_str or "vision_config" in error_str:
                print(f"Detected config loading issue, attempting to fix: {e}")
                # Load config manually and fix dict issues
                from transformers import AutoConfig
                fixed_config = AutoConfig.from_pretrained(model_id, trust_remote_code=model_init_kwargs.get("trust_remote_code", None))
                fixed_config = fix_config_dict_issues(fixed_config)
                
                # Now load model with fixed config
                model_init_kwargs_fixed = model_init_kwargs.copy()
                model_init_kwargs_fixed['config'] = fixed_config
                # Remove use_cache from model_init_kwargs since config already has it
                model_init_kwargs_fixed.pop('use_cache', None)
                model = model_cls.from_pretrained(model_id, **model_init_kwargs_fixed)
            else:
                raise e
        ...
        if self.beta == 0.0:
            ...
            # Use the same config fixing logic for reference model
            try:
                self.ref_model = model_cls.from_pretrained(model_id, **model_init_kwargs)
            except (AttributeError, TypeError) as e:
                error_str = str(e)
                if "'dict' object has no attribute 'to_dict'" in error_str or "text_config" in error_str or "vision_config" in error_str:
                    print(f"Detected config loading issue for ref_model, using fixed config")
                    # Use the same fixed config from main model if available
                    if fixed_config is not None:
                        ref_model_init_kwargs = model_init_kwargs.copy()
                        ref_model_init_kwargs['config'] = fixed_config
                        # Remove use_cache from model_init_kwargs since config already has it
                        ref_model_init_kwargs.pop('use_cache', None)
                        self.ref_model = model_cls.from_pretrained(model_id, **ref_model_init_kwargs)
                    else:
                        # Create a new fixed config for ref model
                        from transformers import AutoConfig
                        ref_config = AutoConfig.from_pretrained(model_id, trust_remote_code=model_init_kwargs.get("trust_remote_code", None))
                        ref_config = fix_config_dict_issues(ref_config)
                        ref_model_init_kwargs = model_init_kwargs.copy()
                        ref_model_init_kwargs['config'] = ref_config
                        # Remove use_cache from model_init_kwargs since config already has it
                        ref_model_init_kwargs.pop('use_cache', None)
                        self.ref_model = model_cls.from_pretrained(model_id, **ref_model_init_kwargs)
                else:
        ...
        if processing_class is None:
            ...
            try:
                ...
                if "'dict' object has no attribute 'to_dict'" in str(e):
                    print(f"Detected config loading issue for processing_class, attempting workaround: {e}")
                    # Try to load with the fixed config if available
                    if 'config' in model_init_kwargs:
    ...
    def _generate_and_score_completions(self, inputs: dict[str, Union[torch.Tensor, Any]], model) -> dict[str, Union[torch.Tensor, Any]]:
        ...
        for i, (reward_func, reward_processing_class) in enumerate(
            ...
            if isinstance(reward_func, PreTrainedModel):
                ...
                output_reward_func = reward_func(prompts=prompts, completions=completions, model=model, **reward_kwargs)
