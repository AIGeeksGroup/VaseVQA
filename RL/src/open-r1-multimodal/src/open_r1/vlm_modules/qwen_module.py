...
class Qwen2VLModule(VLMBaseModule):
    ...
    def prepare_model_inputs(self, processing_class, prompts_text, images, return_tensors="pt", padding=True, padding_side="left", add_special_tokens=False):
        ...
        if len(images) > 0:
            ...
            # Fix: calculate how many images per sample, then correctly group image_grid_thw
            num_samples = len(prompts_text)
            images_per_sample = len(images) // num_samples
            
            # Create an additional_output item for each sample
            additional_output = []
            for i in range(num_samples):
                start_idx = i * images_per_sample
                end_idx = start_idx + images_per_sample
                sample_image_grid_thw = prompt_inputs['image_grid_thw'][start_idx:end_idx]
                additional_output.append({'image_grid_thw': sample_image_grid_thw})
        else:
            prompt_inputs = processing_class(
                text=prompts_text,
                return_tensors=return_tensors,
    ...
    def get_question_template(task_type: str):
        # Import skill library
        try:
            from open_r1.stardojo_skills import get_skill_library
            skill_library = get_skill_library()
            skill_library_json = json.dumps(skill_library, ensure_ascii=False, indent=2)
            # Escape braces to avoid format conflicts
            skill_library_json = skill_library_json.replace("{", "{{").replace("}", "}}")
        except ImportError:
            skill_library_json = "[]"
        
        match task_type:
            case "stardojo":
                return f"""Based on the starting and ending game states and the given task, predict the sequence of actions.
...
You should select actions from the valid action set above. First output the thinking process in <think> </think> tags and then output the final answer in <answer> </answer> tags. Output the final answer in JSON format."""
            case "stardojo1":
                return f"""Based on the starting game state and the given task, predict the sequence of actions.
...
You should select actions from the valid action set above. First output the thinking process in <think> </think> tags and then output the final answer in <answer> </answer> tags. Output the final answer in JSON format."""
            case "stardojo2":
                return f"""Based on the starting and ending game states, predict the sequence of actions.
    ...
    def select_reward_func(func: str, task_type: str):
        if func == "accuracy":
            match task_type:
                ...
                case "stardojo" | "stardojo1" | "stardojo2":
                    from open_r1.grpo_jsonl import stardojo_action_rewards
                    return stardojo_action_rewards
                case _:
